...
class Yolov1(nn.Module):
    ...
    def _create_conv_layers(self, architecture):
        ...
        for x in architecture:
            if type(x) == tuple:
                layers += [
                    CNNBlock(in_channels=in_channels, out_channels=x[1], kernel_size=x[0], stride=x[2], padding=x[3])
                ]
                in_channels = x[1]
            elif type(x) == str:
    ...
    def _create_fcs(self, split_size, num_boxes, num_classes):
        ...
        # According to original paper this should be
        ...
        return nn.Sequential(
            nn.Flatten(),
            nn.Linear(in_features=1023 * S * S, out_features=496),
            nn.Dropout(0.0),
            nn.LeakyReLU(0.1),
